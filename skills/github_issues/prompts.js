 ...
 * 必须做到的事。
 *
 * ⚠️ 这里**不再要求"每条都带网址"**：结果已经由界面上的**可点击卡片**承载，
 * 模型逐条重复反而会和卡片叠成两份一样的清单（用户实测反馈）。
 * 模型要做的是说清**范围**与**条数**，然后把列表交给卡片。
 */
export const REQUIREMENTS = Object.freeze([
  ...
  {
    id: 'match_count',
    rule: '必须说明共匹配多少条（"共 N 条"）',
    check: (/** @type {string} */ text) => /共\s*\d+\s*条/.test(text),
  },
  {
    id: 'no_item_dump',
    rule: '不要逐条重复卡片里已有的列表',
    check: (/** @type {string} */ text) =>
      // 出现 3 个以上 issue 链接 = 又在逐条列了
      (text.match(/https:\/\/github\.com\/\S+\/issues\/\d+/g) ?? []).length < 3,
  },
]);
...
export function githubIssuesSystemBlock() {
  const lines = [
    ...
    '### ⚠️ 列表已经显示给用户了，不要再抄一遍',
    '',
    '技能返回的结果会**自动以可点击列表的形式展示在界面上**（比你能看到的条数更多）。',
    '所以你**只需要一两句话**说清楚两件事：',
    '',
    '- 范围：「最近 N 天」',
    '- 条数：「共 N 条匹配」',
    '',
    '例如：「vllm-project/vllm 最近 7 天有人讨论的 issue，共 340 条，下面是最新的 10 条。」',
    '',
    '**绝对不要**把每一条的标题和链接再列一遍——那会和界面上的列表叠成两份一模一样的东西。',
    '（用户实测反馈过这个问题：同一批 issue 被展示了两遍。）',
    '',
    '### 绝对不要说',
    '',
