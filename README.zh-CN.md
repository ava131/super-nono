...
它带着一套带权限闸门的技能系统：天气、行情、自选股、领域 issue 追踪。
...
| 我自选股都怎么样了 | 批量扫一遍自选 |
| 帮我找一下 vllm 最新的 issue | 本地解析仓库短名，**只发 1 次请求**，再渲染成可点击列表 |
| AI infra 最近有什么新问题 | 领域解析成 3 个仓库；**领域不认识时绝不猜** |
| （说一个没收录的领域） | 明说没收录，并列出**它认识哪些**——**0 次网络请求** |
...
| `github_issues` | 某领域**最近有人讨论**的 issue（vLLM / SGLang / …），渲染成可点击列表 | L1 只读 |
...
│   └── github_issues/      领域 → issue 列表（本地领域表，不猜）
